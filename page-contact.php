    ...
    <div class="contact">
        ...
        <div class="contact__form">
            <?php if (isset($email_status)) : ?>
                <p class="alert alert--<?= $email_status === true ? 'success' : 'error'; ?>" role="alert">
                    <?= $email_status === true ? 'Votre message a été envoyé avec succès!' : 'Une erreur est survenue lors de l\'envoi du message...'; ?>
                </p>
                <?php if ($email_status !== true): ?>
                    <div class="flex-centered">
                        <a href="<?= get_page_link(); ?>" class="link">Réessayer</a>
                    </div>
                <?php endif; ?>
            <?php else: ?>
                <form class="form" action="<?php the_permalink() ?>" method="post" id="contactForm" novalidate>
                    <div class="form__group">
                        ...
                        <?php if (isset($_POST['submitted']) && ($email_error !== '')) : ?>
                    ...
                    <div class="form__group">
                        ...
                        <?php if (isset($_POST['submitted']) && ($object_error !== '')) : ?>
                            <p class="invalid-feedback"><?= $object_error ?></p>
                        <?php endif; ?>
                    </div>
                    <div class="form__group">
                        <label for="messageInput" class="form__group__label">Votre message</label>
                        <textarea name="message" id="messageInput" cols="30" rows="10" class="form__group__input form__group__input--textarea <?= (isset($message_error) && $message_error !== '') ? 'is-invalid' : ''; ?>" placeholder="<?php the_field('message_placeholder') ?>"><?= isset($message_value) ? $message_value : ''; ?></textarea>
                        <?php if (isset($_POST['submitted']) && ($message_error !== '')) : ?>
